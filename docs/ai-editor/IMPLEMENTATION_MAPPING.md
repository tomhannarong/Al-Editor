...
| Shell-free bounded ffprobe execution | `packages/media-catalog/src/ffprobe.ts` | verified on `e2bd213a`, CI run `32806749817`, job `97678251159`; direct argv with `shell:false`, timeout/stdout/stderr limits, fail-closed exit handling, strict JSON handoff to native-stream normalizer |
...
Managed original materialization closes the immutable-original ownership gap for local ingest. A registered asset can be copied into managed storage at a deterministic SHA-256 path using an exclusive temporary file and create-if-absent hard-link publication. Existing content is reused only after digest/size verification; corrupted content or changed source bytes fail closed before managed location state is published. Successful final content is marked read-only as an additional application-level guard.

The ffprobe process boundary closes the execution-authority gap without changing metadata semantics. Media paths are passed as argv data, no shell is enabled, runtime and captured output are bounded, non-zero/timeout/overflow states fail closed, and only successfully parsed JSON reaches the already verified native-PTS normalizer.
...
Phase 0 is complete: 22/22 standalone items verified. Phase 1 is now 7/14 verified. The next run should audit the smallest end-to-end immutable-ingest orchestration gap across the seven verified primitives before adding new metadata or derivative capabilities.