...
| Model/prompt/model-artifact registry | versioned registry convention | next |
| Concrete FFmpeg v2 adapter | worker adapter | waits canonical render/walking-skeleton dependencies |

Canonical timing remains integer frames + rational FPS and native PTS + rational stream time base. Delivery/style policy cannot become timing authority; FFmpeg/FFprobe remains final media measurement authority.
