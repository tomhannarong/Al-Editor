...
| Durable filesystem-to-PostgreSQL ingest composition | `infra/verify-postgres-durable-ingest-runtime.mts` + selective local-stack workflow | verified on `fea5a180`; real local-stack run `32819714185`, job `97715097100`; confined local source → verified managed original → deterministic ffprobe/native timing → atomic PostgreSQL persistence → idempotent re-ingest |
...
P1-08 composes the filesystem/media primitives without creating a second identity or timing implementation. P1-09 ensures no durable callback is possible until all deterministic media validation succeeds. P1-10 provides the atomic PostgreSQL transaction. P1-11 now proves those three layers work together against a real filesystem and real PostgreSQL while preserving idempotent content identity and native timing authority.
...
Phase 0 is complete: 22/22 standalone items verified. Phase 1 is now 11/14 verified. The next run should audit the three remaining Phase-1 items and select the smallest dependency-correct catalog/ingest gap before introducing Phase-2 derivatives.
