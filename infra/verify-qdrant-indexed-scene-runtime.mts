...
function document(): IndexedSceneDocument {
  return {
    ...
    documentId: 'indexed-scene:runtime-scene-1', revisionId: 'indexed-scene-revision:runtime-v1',
    source: {
      sceneSetId: 'scene-set:runtime-phase4', sceneSetRevisionId: 'scene-set-revision:runtime-v1', sceneId: 'scene-runtime-1',
      assetId, streamId: `${assetId}:stream:0`, streamIndex: 0,
    ...
    embedding: {
      embeddingRevisionId: 'embedding-revision:runtime-v1', modelId: 'deterministic-runtime-fixture', modelVersion: '1.0.0',
...
if (first.document.embedding.vectorSha256 !== computeIndexedSceneVectorSha256(vector)) {
  throw new Error('immutable source-vector digest evidence changed during Qdrant upsert');
}
if (first.vector.length !== vector.length) throw new Error('Qdrant readback vector dimensions changed');
...
if (readback.document.source.sourceTimeBase.numerator !== 1 || readback.document.source.sourceTimeBase.denominator !== 90_000) {
  throw new Error('Qdrant payload did not preserve normalized rational source time base');
}
if (readback.document.embedding.vectorSha256 !== computeIndexedSceneVectorSha256(vector)) {
  throw new Error('Qdrant payload changed immutable source-vector digest evidence');
}

let conflictRejected = false;
try {
  await store.upsertDocument({ ...document(), representationText: 'Conflicting text must not overwrite immutable revision.' }, vector);
} catch (error) {
...
console.log(JSON.stringify({
  ok: true, qdrantUrl, collectionName, pointId: first.pointId, idempotentReuse: !second.created,
  sceneId: readback.document.source.sceneId, normalizedTimeBase: readback.document.source.sourceTimeBase,
  sourceVectorSha256: readback.document.embedding.vectorSha256,
  qdrantStoredVectorSha256: computeIndexedSceneVectorSha256(readback.vector),
  qdrantVectorIsRebuildableState: computeIndexedSceneVectorSha256(readback.vector) !== readback.document.embedding.vectorSha256,
  conflictRejected,
}, null, 2));
