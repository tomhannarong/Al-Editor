...
QDRANT_HTTP_PORT_VALUE="${QDRANT_HTTP_PORT:-6333}"
...
wait_for_container_health() {
  ...
  while true; do
    ...
    case "$status" in
      healthy) echo "PASS: $service is healthy"; return 0 ;;
      unhealthy)
        echo "ERROR: $service became unhealthy" >&2
        docker compose -f "$COMPOSE_FILE" logs --no-color "$service" >&2 || true
        return 1 ;;
      no-healthcheck) echo "ERROR: $service has no container healthcheck" >&2; return 1 ;;
    esac
    now="$(date +%s)"
...
wait_for_qdrant_http() {
  local started_at now container_id
  started_at="$(date +%s)"
  while true; do
    container_id="$(docker compose -f "$COMPOSE_FILE" ps -q qdrant)"
    if [[ -z "$container_id" ]]; then
      echo "ERROR: qdrant has no running container" >&2
      return 1
    fi
    if curl --fail --silent --show-error --max-time 2 "http://127.0.0.1:${QDRANT_HTTP_PORT_VALUE}/healthz" >/dev/null 2>&1; then
      echo "PASS: qdrant HTTP health endpoint succeeded"
      return 0
    fi
    now="$(date +%s)"
    if (( now - started_at >= WAIT_SECONDS )); then
      echo "ERROR: timed out waiting for Qdrant HTTP health after ${WAIT_SECONDS}s" >&2
      docker compose -f "$COMPOSE_FILE" logs --no-color qdrant >&2 || true
      return 1
    fi
    sleep "$POLL_SECONDS"
  done
